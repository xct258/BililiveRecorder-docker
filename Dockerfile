...
RUN apt-get update \
    ...
    && echo     wget -O 7zz.tar.xz https://www.7-zip.org/a/7z2409-linux-x64.tar.xz >> /root/tmp/1.sh \
    && echo     wget -O BililiveRecorder-CLI.zip https://github.com/BililiveRecorder/BililiveRecorder/releases/latest/download/BililiveRecorder-CLI-linux-x64.zip >> /root/tmp/1.sh \
    && echo elif [[ $arch == *"aarch64"* ]]; then >> /root/tmp/1.sh \
    && echo     wget -O 7zz.tar.xz https://www.7-zip.org/a/7z2409-linux-arm64.tar.xz >> /root/tmp/1.sh \
    && echo     wget -O BililiveRecorder-CLI.zip https://github.com/BililiveRecorder/BililiveRecorder/releases/latest/download/BililiveRecorder-CLI-linux-arm64.zip >> /root/tmp/1.sh \
